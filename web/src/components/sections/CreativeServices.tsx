...
import { Video, Monitor, Layers, Check, type LucideIcon } from 'lucide-react';
...
const ICONS: Record<string, LucideIcon> = { Video, Monitor, Layers };

/** Servicios del track creativo (pilares media + tech). */
export function CreativeServices() {
    const { t, language } = useLanguage();
    const services = MOCK_SERVICES
        .filter((s) => s.pilar === 'media' || s.pilar === 'tech')
        ...
        <section className="relative bg-black py-20 sm:py-28">
            <div className="container mx-auto px-4 sm:px-6">
                <Reveal as="p" className="mb-3 font-mono text-[11px] font-black uppercase tracking-[0.3em] text-violet-300/80">
                    {t('estudio', 'services_eyebrow')}
                </Reveal>
                <RevealHeading
                    text={t('estudio', 'services_title')}
                    as="h2"
                            ...
                            <Reveal key={s.id} delay={i * 0.1} className="h-full">
                                <div className="glass-card flex h-full flex-col gap-5 rounded-3xl p-7 sm:p-8">
                                    <div className="flex items-center justify-between">
                                        <span className="grid h-12 w-12 place-items-center rounded-2xl border border-violet-400/25 bg-violet-500/10 text-violet-200">
