...
export function StudioExperience() {
  ...
  return (
    <main id="contenido-principal" className="bg-transparent pb-28 pt-36 sm:pt-44">
      ...
      <section className="px-5 py-24 sm:px-8 lg:px-12 lg:py-32"><div className="mx-auto grid max-w-[1480px] gap-12 lg:grid-cols-[1fr_1fr] lg:items-end"><div><p className="font-mono text-[10px] uppercase tracking-[.18em] text-accent">02 · {isEs ? "Del concepto al navegador" : "From concept to browser"}</p><h2 className="mt-5 text-[clamp(2.8rem,5vw,5.8rem)] font-medium leading-[.95] tracking-[-.055em] text-foreground">{isEs ? "Una sola intención, de la dirección al código." : "One intent, from direction to code."}</h2></div><div className="lg:justify-self-end"><p className="max-w-xl text-lg leading-7 text-[#F3F0E8]/55 light:text-muted-foreground">{isEs ? "Puedo explorar la forma, prototipar la interacción y construir la experiencia final. Eso reduce la distancia entre lo que se imagina y lo que realmente llega a producción." : "I can explore the form, prototype the interaction and build the experience. That reduces the distance between what is imagined and what actually ships."}</p><div className="mt-7 flex flex-wrap gap-6"><Link href={localePath(language, "/casos-de-exito")} className="inline-flex items-center gap-2 border-b border-white/20 light:border-[rgb(var(--ink-rgb)/0.2)] pb-1 text-sm text-foreground">{isEs ? "Ver archivo creativo" : "View creative archive"}<ArrowUpRight className="h-4 w-4" /></Link><Link href={localePath(language, "/aplicar")} className="inline-flex items-center gap-2 text-sm text-[#F3F0E8]/55 light:text-muted-foreground">{isEs ? "Proponer una experiencia" : "Propose an experience"}<ArrowUpRight className="h-4 w-4" /></Link></div></div></div></section>
    </main>
  );
}
